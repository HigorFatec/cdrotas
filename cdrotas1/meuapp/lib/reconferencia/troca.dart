...
class TrocasScreen2 extends StatefulWidget {
  final Troc2? trocaSelecionada;

  const TrocasScreen2({Key? key, this.trocaSelecionada}) : super(key: key);

  @override
  // ignore: library_private_types_in_public_api
  _TrocasScreenState2 createState() => _TrocasScreenState2();
}

class _TrocasScreenState2 extends State<TrocasScreen2> {
  final List<Troca2> _trocas = [];
  ...
  Future<void> _carregarTrocas() async {
    final snapshot =
        await FirebaseFirestore.instance.collection('trocas2').get();
  ...
  void _removerTroca(int index) async {
    if (index >= 0 && index < _trocas.length) {
      ...
      final docRef =
          FirebaseFirestore.instance.collection('trocas2').doc(docId);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Reconferencia Trocas'),
      ),
      body: Container(
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        suffixIcon: GestureDetector(
                          onTap: () {
                            Navigator.pushNamed(context, '/listtroca2');
                          },
